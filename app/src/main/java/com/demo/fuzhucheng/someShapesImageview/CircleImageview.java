...
//        int bitmapSize = Math.min(bitmap.getWidth(), bitmap.getHeight());
         ...
         * 三、要注意的是：我们怎么弄出圆形的呢？使用Matrix来缩放图片，设置size，按照最小的一边来缩放，缩放到相同比例成为一个圆形，而按照了最小边来缩放，意味最长的那边会丢失，可见是通过裁剪得到的一个圆
         */
    ...
    /**
     * 我们可以知道，如果我们直接用imageview然后引用shape弄成圆形的话，意味着我们在这个imageview的逻辑只能写在fragment等等里面了，而很难去进行逻辑的分层.。而且！！只能用矢量图
